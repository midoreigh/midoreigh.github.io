...
$rocchio = new Rocchio(); 
$cal = file_get_contents('/home/midoreigh/public_html/midoreigh/mido.txt');

$cal = explode("\n", $cal);
for($i = 0; $i < count($cal)-1; $i += 2){
        $rocchio->addDocument($cal[$i], 'positive');
}
$rocchio->train();
//for($i = 1; $i < count($cal); $i += 2) {
//        $rocchio->test($cal[$i], 'positive');
//}
$rocchio->test("tcs is going down", 'negative');
$rocchio->test("mido", 'positive');
$rocchio->getStats();
?>